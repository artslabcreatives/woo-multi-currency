...
class WOOMULTI_CURRENCY_Admin_Analytics {
	...
	public function __construct() {
		...
		/*Orders*/
//		add_filter( 'woocommerce_analytics_orders_select_query', array( $this, 'woocommerce_analytics_orders_select_query' ) );
		/*Orders stats*/
//		add_filter( 'woocommerce_analytics_orders_stats_query_args', array( $this, 'woocommerce_analytics_query_args' ) );
//		add_filter( 'woocommerce_analytics_orders_stats_select_query', array( $this, 'convert_orders_stats' ) );
		/*Revenue*/
//		add_filter( 'woocommerce_analytics_revenue_query_args', array( $this, 'woocommerce_analytics_query_args' ) );
//		add_filter( 'woocommerce_analytics_revenue_select_query', array( $this, 'convert_orders_stats' ) );
		/*Products*/
//		add_filter( 'woocommerce_analytics_products_query_args', array( $this, 'woocommerce_analytics_query_args' ) );
//		add_filter( 'woocommerce_analytics_products_select_query', array( $this, 'convert_orders_stats' ) );
		/*Products stats*/
//		add_filter( 'woocommerce_analytics_products_stats_query_args', array( $this, 'woocommerce_analytics_query_args' ) );
//		add_filter( 'woocommerce_analytics_products_stats_select_query', array( $this, 'convert_orders_stats' ) );
		/*Categories*/
//		add_filter( 'woocommerce_analytics_categories_query_args', array( $this, 'woocommerce_analytics_query_args' ) );
//		add_filter( 'woocommerce_analytics_categories_select_query', array( $this, 'convert_orders_stats' ) );
	...
	public function get_total_discount( $order_id ) {
		return get_post_meta( $order_id, '_discount_total', true );
	}
...
//		error_log( __FILE__ . ':' . __LINE__ . "\n" . print_r( $order_data, true ) );
		...
		$order_info = $parent_id ? get_post_meta( $parent_id, 'wmc_order_info', true ) : get_post_meta( $order_id, 'wmc_order_info', true );
		$currency   = get_post_meta( $order_id, '_order_currency', true );
		$rate       = 1;

		if ( isset( $order_info[ $currency ], $order_info[ $default_currency ], $order_info[ $default_currency ]['is_main'] )
		     && $order_info[ $default_currency ]['is_main'] == 1
		     && $order_info[ $currency ]['rate'] > 0 ) {
...
//			$args['extended_info'] = 1;
			...
			$this->args = null;
//			error_log( __FILE__ . ':' . __LINE__ . "\n" . print_r( memory_get_usage() - $start, true ) );
		}
